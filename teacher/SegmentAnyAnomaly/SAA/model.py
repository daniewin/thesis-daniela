...
class Model(torch.nn.Module):
    ...
    def set_property_text_prompts(self, object_prompt,
                object_number,
                k_mask,
    ...
    def ensemble_text_guided_mask_proposal(self, image, object_phrase_list, filtered_phrase_list,
        ...
        if ensemble_boxes != []:
            ...
            """
            if True: #filtered_phrase_list == ['PlaceHolder']:
                image_arr= Image.fromarray(image)
                image_width, image_height = image_arr.size
            
                # Create a drawing context
                draw = ImageDraw.Draw(image_arr)
                
                # Draw each bounding box
                for bbox in ensemble_boxes:
                ...
                output_path = f'/data-fast/winter/isead/38k/birds/SegmentationMask/{filtered_phrase_list[0]}_{np.random.randint(100)}.jpg'
    ...
    def forward(self, image: np.ndarray):
        ...
        #self.defect_min_area = self.defect_min_area
